...
CONFIG_PATH="/cluster/projects/nn9305k/active/330403_001-A4_Tapeworm/analyses/assembly_masurca/20250926/582-1/test_config"
CMD_PATH="/cluster/software/MaSuRCA/4.1.0-GCC-11.3.0/bin/masurca"
set -o pipefail
...
export PATH="/cluster/software/MaSuRCA/4.1.0-GCC-11.3.0/bin/../bin:/cluster/software/MaSuRCA/4.1.0-GCC-11.3.0/bin:$PATH"
save PATH
export PERL5LIB=/cluster/software/MaSuRCA/4.1.0-GCC-11.3.0/bin/../lib/perl${PERL5LIB:+:$PERL5LIB}
save PERL5LIB
NUM_THREADS=32
save NUM_THREADS
log 'Processing pe library reads'
rm -rf meanAndStdevByPrefix.pe.txt
echo 'pe 150 50' >> meanAndStdevByPrefix.pe.txt
rename_filter_fastq 'pe' <(exec expand_fastq '582-1_R1.fastq.gz' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}') <(exec expand_fastq '582-1_R2.fastq.gz' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}' ) > 'pe.renamed.fastq'
...
JF_SIZE=`ls -l *.fastq | awk '{n+=$5}END{s=int(n/50); if(s>8240000000)printf "%.0f",s;else print "8240000000";}'`
save JF_SIZE
perl -e '{if(int('$JF_SIZE')>8240000000){print "WARNING: JF_SIZE set too low, increasing JF_SIZE to at least '$JF_SIZE', this automatic increase may be not enough!\n"}}'
...
quorum_error_correct_reads  -q $(($MIN_Q_CHAR + 40)) --contaminant=/cluster/software/MaSuRCA/4.1.0-GCC-11.3.0/bin/../share/adapter.jf -m 1 -s 1 -g 1 -a 3 -t 32 -w 10 -e 3 -M  quorum_mer_db.jf pe.renamed.fastq --no-discard -o pe.cor.tmp --verbose 1>quorum.err 2>&1 && mv pe.cor.tmp.fa pe.cor.fa || fail Error correction of PE reads failed. Check pe.cor.log.
...
runCA -s runCA.spec stopAfter=initialStoreBuilding -p genome -d CA  doFragmentCorrection=0 doOverlapBasedTrimming=0 doExtendClearRanges=0 ovlMerSize=30 superReadSequences_shr.frg      1> runCA0.out 2>&1
...
runCA -s runCA.spec stopBefore=scaffolder -p genome -d CA  doFragmentCorrection=0 doOverlapBasedTrimming=0 doExtendClearRanges=0 ovlMerSize=30 superReadSequences_shr.frg      1> runCA1.out 2>&1

if [ ! -d CA/1-overlapper ]; then
  rm -f CA/0-overlaptrim-overlap/overlap.sh CA/1-overlapper/overlap.sh && 
  runCA -s runCA.spec -p genome -d CA stopBefore=scaffolder doFragmentCorrection=0 doOverlapBasedTrimming=0 doExtendClearRanges=0 ovlMerSize=30 superReadSequences_shr.frg      1>>runCA1.out 2>&1
fi
...
runCA -s runCA.spec stopBefore=scaffolder -p genome -d CA  doFragmentCorrection=0 doOverlapBasedTrimming=0 doExtendClearRanges=0 ovlMerSize=30 superReadSequences_shr.frg      1> runCA2.out 2>&1 
log "Recomputing A-stat for super-reads"
recompute_astat_superreads_CA8.sh genome CA $PE_AVG_READ_LENGTH work1/readPlacementsInSuperReads.final.read.superRead.offset.ori.txt superReadSequences_shr.frg
fi
...
if [ -s $CA_DIR/9-terminator/genome.scf.fasta ];then
  ...
  if [ $NCTG -eq $NSCF ];then
    ...
    if [ -s $CA_DIR/10-gapclose/genome.scf.fasta ];then
      ...
      log 'Gap closing'
      closeGapsLocally.perl --max-reads-in-memory 1000000000 -s 8240000000 --Celera-terminator-directory $CA_DIR/9-terminator --reads-file 'pe.renamed.fastq' --output-directory $CA_DIR/10-gapclose --min-kmer-len 17 --max-kmer-len $(($PE_AVG_READ_LENGTH-5)) --num-threads 32 --contig-length-for-joining $(($PE_AVG_READ_LENGTH-1)) --contig-length-for-fishing 200 --reduce-read-set-kmer-size 21 1>gapClose.err 2>&1
...
if [ -s $CA_DIR/$TERMINATOR/genome.scf.fasta ];then
  ...
  log 'Removing redundant scaffolds'
    ...
    deduplicate_contigs.sh $CA_DIR genome 32 $PLOIDY $TERMINATOR 
  fi
  log "Assembly complete, primary scaffold sequences are in $CA_DIR/primary.genome.scf.fasta"
  log "redundant or haplotype variant scaffold sequences are in $CA_DIR/alternative.genome.scf.fasta"
else
  fail "Assembly stopped or failed, see $CA_DIR.log"
fi
log "All done, final assembly is in $CA_DIR/primary.genome.scf.fasta"
log "Final stats for $CA_DIR/primary.genome.scf.fasta"
ufasta n50 -a $CA_DIR/primary.genome.scf.fasta
